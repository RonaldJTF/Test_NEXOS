...
@Log4j2
@ControllerAdvice
public class NexosExceptionGlobalHandler {
    @ExceptionHandler(NexosException.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    @ResponseBody
    public ResponseEntity<?> handleException(NexosException ex) {
        Trace.logError(Methods.class.getName(), Methods.getCurrentMethodName(Methods.class), ex);
        return Methods.handleException(ex);
    }
}